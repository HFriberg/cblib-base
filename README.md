...
Additional files are included for administrative purposes.
  ...
  ./scripts/
      ./admin/
          ...
          print-bibtable.py             Print table of reference citations based on ''ref.bib''.
          print-reftable.py		Print table of references based on ''ref.csv''.
          print-stattable.py		Print table of instance statistics based on ''stat.csv''.
          update-autogen-tags.py	Create/update autogenerated comments in CBF formatted instances.
          update-stattable.py		Analyse a solution file and update ''stat.csv'' when appropriate.
...
Usage examples:
  ...
  Solve problems listed in 'TEST1' (with newline as seperator)
    python scripts/run.py runmosek -l tests/TEST1

Both commands will output time and solution summary onto the screen. Solution 
files are written to the directory of the file specified as argument.
...
Usage examples:
  ...
  Summary of solutions to problems listed in 'TEST1' (assuming that solutions
  are stored with '.sol' postfix to problem names)
    python scripts/summary.py -l tests/TEST1
